...
export function BtnSelect({status, a, verify, history, pokemon, i, playVoice, handleSetOption}) {
    const button = classNames(
        ...
        {
            "bg-green-500 pointer-events-none focus:bg-green-500 hover:bg-green-500": status === a && verify === a,
            "bg-green-500 pointer-events-none opacity-60 focus:bg-green-500 hover:bg-green-500": history && verify && pokemon.name === a && verify !== a,
            "bg-red-700 pointer-events-none focus:bg-red-700 hover:bg-red-700": status === 'failed' && verify && verify === a && pokemon.name !== a,
            "pointer-events-none opacity-60 bg-transparent focus:bg-transparent hover:bg-transparent": verify && pokemon.name !== a && verify !== a
        }
    )
